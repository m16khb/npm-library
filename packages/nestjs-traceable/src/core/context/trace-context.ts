...
import { AsyncLocalStorageManager } from './async-storage';
...
export class TraceContextManager implements ITraceContextManager {
  ...
  runAsync<T>(context: ITraceContext, fn: () => Promise<T>): Promise<T> {
    return this.storage.runAsync(context, fn);
  }

  /**
   * 새로운 자식 컨텍스트를 생성한다.
   * 현재 컨텍스트가 없으면 undefined를 반환한다.
   */
